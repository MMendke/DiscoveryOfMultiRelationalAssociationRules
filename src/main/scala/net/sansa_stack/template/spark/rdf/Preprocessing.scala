...
{
      private val sc = TripleReader.sparkContext
      private var propDomRan = new HashMap[String, LinkedHashSet[String]]()
      ...
      def run(spark: SparkSession, filteredRDD: RDD[OWLAxiom], propDetailRDD: RDD[OWLAxiom])
      {
            ...
            // Converting Rdd[OwlAxioms] into RDD[Set(String,String,String)]
            ...
            var domainSeedRdd=prepareSeed(domainPropRdd)//Domain
            
            val rangePropRdd=propDetailRDD.filter(f=>(f.isInstanceOf[OWLObjectPropertyRangeAxiom]))
            var rangeSeedRdd=prepareSeed(rangePropRdd)//Range
            ...
            val domRanSeedRdd= (domainSeedRdd.join(rangeSeedRdd)).map(f=>
             ...
             //println("domRanSeedRdd:")
             //domRanSeedRdd.foreach(println)
             
             // to remove recursive atoms from domainSeedRdd and rangeSeedRdd
            ...
            //Forming clusters based on seeds
            ...
            //tablesToBeCreated = clusterofAllRDD.flatMap(f=>f).distinct()
            //tablesToBeCreated.foreach(println)
            ...
            else
            {
                  ...
                  //clusterofAllRDD.foreach(println)
                  
                  //Forming Combinations of Three Atoms Randomly.
                  val comboPredAtom = comboOfPredAtom(clusterofAllRDD,true)
                 	overlapTables(domRanSeedRdd,comboPredAtom)
      ...
      def verticalPartitioning(filteredRDD: RDD[org.semanticweb.owlapi.model.OWLAxiom]): RDD[(String, Iterable[(String, String)])]= 
          {
                 val partitionRDD =filteredRDD.map(f=>f.toString().split(" "))
                .map(f => {
                ...
                //.groupByKey() // group based on key
               /*println("vertical partition")
               partitionRDD.foreach(println)*/
               partitionRDD
           }
      def fetchTableName(partitionRDD: RDD[(String, Iterable[(String, String)])])//: RDD[Iterable[(String, Int)]]=
      {
            for (p<-partitionRDD)
            {
      ...
      def overlapTables(domRanSeedRdd: RDD[(String, LinkedHashSet[String])],comboPredAtom: RDD[List[String]])
      {
            ...
            for(c<-combo)
            {
                  val common = tableDetail(c(0)).intersect(tableDetail(c(1)))
                  overLap((c(0),c(1)))=common
            }
      }
      def prepareSeed(propDetailRDD: RDD[OWLAxiom]): RDD[(String, LinkedHashSet[String])]=
      {
             val seedRDD = propDetailRDD.map(f=>f.toString().split(" "))
            .map(f => {
                        val key =  f(0).substring(f(0).indexOf("(")+1)
      ...
      def localSensitiveHashing(spark: SparkSession,predWithTriples:RDD[(String,scala.collection.mutable.Set[(String, String, String)])],uniqueByPred: RDD[(String, LinkedHashSet[String])]): RDD[List[String]]=
      {
        ...
        val hashingTF = new HashingTF()
          .setInputCol("values").setOutputCol("features").setNumFeatures(20)
        ...
        val result = dffilter.filter($"datasetA.id".isNotNull).filter($"datasetB.id".isNotNull)
